...
def _save_symbol_cache(nse_syms, bse_syms):
    try:
        _SYMBOL_CACHE.write_text(
            _json.dumps({"ts": time.time(), "nse": nse_syms, "bse_only": bse_syms})
        )
    except Exception:
        pass
...
def bulk_ohlc(tickers: list[str], period: str = "1y") -> dict[str, pd.DataFrame]:
    ...
    try:
        ...
        print(
            f"    cache: {stats['symbols']} symbols · {stats['days']} days · "
    ...
    if not hist.empty:
        ...
        for sym, g in hist.groupby("symbol", sort=False):
            ...
            df = df.set_index(pd.to_datetime(df["d"]))[["Open", "High", "Low", "Close", "Volume"]]
            result[f"{sym}.NS"] = df
    ...
    if fallback:
        print(
            f"  yfinance fallback for {len(fallback)} tickers "
...
# ── Darvas Box ─────────────────────────────────────────────────────────────────
# The box-detection loop itself (darvas_box_core) is shared with the other 4
# full_*_market_scan.py files via stock_utils.py -- verified line-identical.
# This wrapper's column lookup (incl. the CH_TRADE_* bhavcopy fallback),
# rounding (2dp), and output dict shape stay local: they're genuinely
# different per market (JP/KR round to 0dp and use different key names) and
# this repo's daily mailer already depends on this file's exact shape.
from stock_utils import darvas_box_core, compute_golden_crossover  # noqa: E402
...
def fundamental_scan(symbol: str, yf_suffix: str = ".NS") -> dict:
    ...
    try:
        ...
        return {
            ...
            "qualifies_bc": False,
            "ROIC_%": None,
            "Earnings_Yield_%": None,
            "Sales_Growth_YoY_%": None,
    ...
    if inc is not None:
        ...
        fcf_pos = (
            (fcf_s[0] > 0)
            if fcf_s
            else ((ocf_s[0] - abs(capex_s[0])) > 0 if (ocf_s and capex_s) else False)
        )
        cc_bits = [
        ...
        out["qualifies_mf"] = bool(
            roic and roic > 25 and ey and ey > 15 and bv and bv > 0 and mcap_cr > 15
        )
    ...
    if inc_q is not None and len(inc_q.columns) >= 5:
        ...
        profit_g = (
            ((ni_q0 - ni_q4) / abs(ni_q4) * 100) if (ni_q0 and ni_q4 and ni_q4 != 0) else None
        )
        ni_cr = ni_q0 / 1e7 if ni_q0 else None
        out["qualifies_bc"] = bool(
            sales_g and sales_g > 15 and profit_g and profit_g > 20 and ni_cr and ni_cr > 1
        )
...
def save_excel(all_rows, darvas_rows, fund_rows, triple_rows, six_screen_rows=None, tag="indian"):
    ...
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        ...
        write_sheet(
            [r for r in fund_rows if r.get("Piotroski_Strong") == "YES"],
            "Piotroski_Strong",
            sort_col="Piotroski_Score",
            sort_asc=False,
        )
        write_sheet(
            [r for r in fund_rows if r.get("CoffeeCan") == "PASS"],
            "Coffee_Can",
            sort_col="Revenue_CAGR_%",
            sort_asc=False,
        )
        write_sheet(
            [r for r in fund_rows if r.get("MagicFormula") == "PASS"],
            "Magic_Formula",
            sort_col="ROIC_%",
            sort_asc=False,
        )
        write_sheet(
            [r for r in fund_rows if r.get("BullCartel") == "PASS"],
            "Bull_Cartel",
            sort_col="Profit_Growth_YoY_%",
            sort_asc=False,
        )
        write_sheet(
            [r for r in all_rows if r.get("GC_Signal") == "GOLDEN_CROSS"],
            "Golden_Crossover",
            sort_col="DMA_Gap%",
            sort_asc=False,
        )
        ...
        write_sheet(
            six_screen_rows or [], "Multi_Screen_Hits", sort_col="Screens_Passed", sort_asc=False
        )
...
def main(nse_only: bool = False, top: int = 0, run_scans: bool = True, workers: int = MAX_WORKERS):
    ...
    all_fund_symbols = [(sym, suf) for yf_tkr, (sym, suf) in yf_map.items() if yf_tkr in ohlc_data]
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if res.get("f_strong"):
                        sc_counts["piotroski"] += 1
                    if res.get("qualifies_cc"):
                        sc_counts["cc"] += 1
                    if res.get("qualifies_mf"):
                        sc_counts["mf"] += 1
                    if res.get("qualifies_bc"):
                        sc_counts["bc"] += 1
                    ...
                    if done % 100 == 0 or done == len(all_fund_symbols):
                        print(
                            f"    {done}/{len(all_fund_symbols)} done  "
    ...
    path = save_excel(all_rows, darvas_rows, fund_rows, triple_rows, six_screen_rows, tag="indian")
    ...
    if six_screen_rows:
        ...
        for r in top:
            print(
                f"    {r['Symbol']:<15} {r['Screens_Passed']}/6 screens  "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--top", type=int, default=0, help="Limit scan to first N tickers (0 = all)"
    )
    parser.add_argument(
        "--no-scans",
        action="store_true",
    ...
    main(nse_only=args.nse_only, top=args.top, run_scans=not args.no_scans, workers=args.workers)
