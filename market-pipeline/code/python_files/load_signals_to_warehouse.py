...
def load_screener_file(conn, path: Path, market: str, chunksize: int = 200_000) -> int:
    ...
    ticker_map, _ = resolve_stock_ids(conn, df["bare_ticker"], market)
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                "CREATE TEMP TABLE stg_screener_signal (LIKE fact_screener_signal INCLUDING DEFAULTS)"
            )
            cur.execute(
                "ALTER TABLE stg_screener_signal DROP COLUMN signal_id, DROP COLUMN loaded_at"
            )
        conn.commit()
...
def load_short_interest(conn, path: Path, chunksize: int = 50_000) -> int:
    ...
    ticker_map, _ = resolve_stock_ids(conn, df["symbol"], "usa")
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                "CREATE TEMP TABLE stg_short_interest (LIKE fact_short_interest INCLUDING DEFAULTS)"
            )
            cur.execute(
                "ALTER TABLE stg_short_interest DROP COLUMN short_interest_id, DROP COLUMN loaded_at"
            )
        conn.commit()
...
def load_insider_transactions(conn, path: Path, chunksize: int = 50_000) -> int:
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                "CREATE TEMP TABLE stg_insider_transaction (LIKE fact_insider_transaction INCLUDING DEFAULTS)"
            )
            cur.execute(
                "ALTER TABLE stg_insider_transaction DROP COLUMN insider_txn_id, DROP COLUMN loaded_at"
            )
        conn.commit()
        ...
        conn.commit()
        log(
            f"  inserted {inserted:,} new rows into fact_insider_transaction (batch_id={batch_id}) "
...
JOBS = {
    "short_interest_us": lambda conn: load_short_interest(
        conn, CACHE_DIR / "short_interest_us.parquet"
    ),
    "insider_transactions_us": lambda conn: load_insider_transactions(
    ...
    "screener_JP": lambda conn: load_screener_file(
        conn, CACHE_DIR / "factorial_screener_signals_JP_technical.parquet", "japan"
    ),
    "screener_CN": lambda conn: load_screener_file(
        conn, CACHE_DIR / "factorial_screener_signals_CN_technical.parquet", "china"
    ),
    "screener_US": lambda conn: load_screener_file(
        conn, CACHE_DIR / "factorial_screener_signals_us.parquet", "usa"
    ),
}
