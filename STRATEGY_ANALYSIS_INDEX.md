...
> **⚠️ RECONCILED 2026-07-14 — do not act on this document as written.**
> The headline claims below ("Piotroski dominates all 7 markets", "Darvas+CCC = 100% win rate", the Japan/UK/Germany screen projections, the +1.7% reallocation uplift) are a **variance observation on a 272-stock sample**, never validated against the repo's actual rigorous backtest. A separate, real point-in-time backtest (`herrrickshaw/market-screener-backtests`, SEC EDGAR + yfinance PIT data, fold-cross-validated) reaches the **opposite** conclusion for the US (Piotroski standalone is *inverted* there) and shows Piotroski only works as a Darvas-breakout confirmation overlay in most other markets, not as a standalone quality filter. The Phase-1 full-universe validation this doc calls "ready to start" was **never run**.
> **See [`STOCK_ANALYSIS_MASTER_SUMMARY.md`](STOCK_ANALYSIS_MASTER_SUMMARY.md) for the reconciled, validated per-market recommendations.** Treat everything below as a superseded hypothesis, kept for history only.

**Status**: 🔴 SUPERSEDED — see reconciliation banner above | Original claim: ✅ Analysis Complete | 🚀 Ready for Phase 1 Implementation  
**Date**: July 6, 2026  
...
*Status: 🔴 SUPERSEDED (2026-07-14) — the 272-stock "dominance" claim and the 100% win-rate figure did not hold up against the repo's real PIT backtest. Confidence downgraded from the original HIGH/MEDIUM ratings above to: do-not-use.*

**Next step: Read [`STOCK_ANALYSIS_MASTER_SUMMARY.md`](STOCK_ANALYSIS_MASTER_SUMMARY.md) for the reconciled per-market recommendations instead of running Phase 1 as originally scoped here.**
