...
find_duplicates() {
    ...
    echo ""
    if [ -d ~/Downloads ]; then
        find ~/Downloads -type f -size +10M 2>/dev/null | awk '{print $0}' | while read file; do
            md5="$(md5 -q "$file" 2>/dev/null)"
            echo "$md5 $file"
        done | sort | awk 'NR==1 { current=$1; count=1; file=$2; next } $1==current { count++; if (count==2) print file; print $2 } $1!=current { current=$1; file=$2; count=1 }' | head -20
    fi
    echo ""
    read -p "Press Enter to continue..."
}
